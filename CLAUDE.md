...
forces the CI token to carry `Zone > Workers Routes > Edit` on top of the
...
server logging raw input would be storing real IBANs and card numbers — a
liability regardless of intent. `packages/_shared/transport.test.mjs` asserts no
raw value reaches telemetry. That is the test least worth breaking silently.
...
is worth it — faster cold starts, no supply chain.

## Architecture

`packages/_shared/` holds everything reusable:

- `http.mjs` — stateless MCP over `fetch`, plus the exported `dispatch` both
  transports share. Telemetry is emitted here, so every server gets it free.
- `telemetry.mjs` — one row per request into Analytics Engine. Read the privacy
  note before adding fields.
- `stdio.mjs` — same dispatch over stdio. Note stdio traffic is **invisible by
  design**: it runs on the user's machine and deliberately does not phone home.
...
nothing here uses. No client IP filtering — GitHub runner IPs rotate.
...
SQL API will 401 for no obvious reason. Back-to-back SQL queries also 429;
space them.

**Nothing has quotas yet.** A single agent loop can fire 30–80 tool calls per
